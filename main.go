...
func home(c echo.Context) error {
	...
	dataProjects, errQuery := connection.Conn.Query(context.Background(), "SELECT id, name, start_date, end_date, description, image FROM tb_projects")
	...
	for dataProjects.Next() {
		...
		err := dataProjects.Scan(&each.Id, &each.ProjectName, &each.StartDate, &each.EndDate, &each.Description, &each.Image)
...
func blogDetail(c echo.Context) error {
	...
	errQuery := connection.Conn.QueryRow(context.Background(),
		"SELECT id, name, start_date, end_date, description FROM tb_projects WHERE id=$1", idToInt).Scan(&blogDetail.Id, &blogDetail.ProjectName, &blogDetail.StartDate, &blogDetail.EndDate, &blogDetail.Description)
...
func addProject(c echo.Context) error {
	...
	_, err := connection.Conn.Exec(context.Background(), "INSERT INTO tb_projects (name, start_date, end_date, description, image) VALUES ($1, $2, $3, $4, $5)", projectname, time.Now(), time.Now(), description, image)
...
func updateProject(c echo.Context) error {
	...
	connection.Conn.Exec(context.Background(), "UPDATE tb_projects SET name=$1, description=$2, image=$3 WHERE id=$4", projectname, description, image, idtoInt)
